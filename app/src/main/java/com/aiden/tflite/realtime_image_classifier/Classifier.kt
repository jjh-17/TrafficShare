...
class Classifier(private var context: Context, private val modelName: String) {
    ...
    //private val traffic_labels = mutableListOf<String>()
    private var isInitialized = false

    fun init() {
        model = createGPUModel(context, modelName)
        initModelShape()
        if(modelName == IMAGENET_CLASSIFY_MODEL)
            labels.addAll(FileUtil.loadLabels(context, LABEL_FILE))
        else if(modelName == TRAFFIC_CLASSIFY_MODEL)
            labels.addAll(FileUtil.loadLabels(context, TRAFFIC_FILE))
        isInitialized = true
    }
    ...
    companion object {
        ...
        const val TRAFFIC_CLASSIFY_MODEL = "model_unquant.tflite"
        const val TRAFFIC_FILE = "classes.txt"
    }
}