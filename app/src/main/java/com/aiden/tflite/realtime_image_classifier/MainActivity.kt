...
class MainActivity : AppCompatActivity() {
    private var oldClass: String = "Nothing"
    ...
    private fun initClassifier() {
        ...
        classifier2 = Classifier(this, Classifier.TRAFFIC_CLASSIFY_MODEL)
        try {
            classifier.init()
        ...
        try {
            ...
            Toast.makeText(this, "Can not init Classifier2!!", Toast.LENGTH_SHORT).show()
        }
    }
        ...
        handler?.post {
            if (::classifier.isInitialized && classifier.isInitialized()) {
                ...
                if(output.second >= 0.0) {
                    ...
                    if (::classifier2.isInitialized && classifier2.isInitialized()) {
                        val startTime = SystemClock.uptimeMillis()
                        val output = classifier2.classify(rgbFrameBitmap!!, sensorOrientation)
                        val elapsedTime = SystemClock.uptimeMillis() - startTime
